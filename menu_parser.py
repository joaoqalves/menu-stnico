    ...
    <div class="container">
     ...
     <script>
         ...
         function shareMenu() {{
             ...
             // Find menu for current date
             let menu = null;
             for (const weekMenu of menuData) {{
                 for (const weekRange of weekMenu.weeks) {{
                     const dateStrISO = currentDate.toISOString().split('T')[0];
                     if (weekRange.start <= dateStrISO && dateStrISO <= weekRange.end) {{
                         menu = weekMenu;
                         break;
                     }}
                 }}
             ...
             let shareText = `🍽️ Menú Sant Nicolau - ${{dayName}} ${{dateStr}}\\n\\n`;
             
             if (menu && menu.days && menu.days[dayName]) {{
                 const dayMenu = menu.days[dayName];
                 if (dayMenu.entrant) shareText += `Entrant: ${{dayMenu.entrant}}\\n`;
                 if (dayMenu.main) shareText += `Principal: ${{dayMenu.main}}\\n`;
                 if (dayMenu.dessert) shareText += `Postre: ${{dayMenu.dessert}}\\n`;
             }} else {{
                 shareText += 'Menú no disponible per aquest dia';
             }}
             
             shareText += '\\n\\nCentre Escolar Sant Nicolau, Sabadell';
         ...
         function fallbackShare(text) {{
             ...
             if (navigator.clipboard) {{
                 navigator.clipboard.writeText(text.replace(/\\n/g, '\\n')).then(() => {{
                     alert('Menú copiat al porta-retalls!');
                 }}).catch(() => {{
                     promptShare(text);
                 }});
             }} else {{
         ...
         function promptShare(text) {{
             // Fallback: show text in prompt for manual copying
             ...
             shareWindow.document.write(`
                 <html>
                     ...
                     <body style="font-family: Arial, sans-serif; padding: 20px;">
                         ...
                         <textarea readonly style="width: 100%; height: 150px; margin: 10px 0;">${{text.replace(/\\n/g, '\\n')}}</textarea>
                         <p>Copieu el text i compartiu-lo on vulgueu!</p>
